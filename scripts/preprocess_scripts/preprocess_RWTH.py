...
def preprocess(path):
    ...
    tqdm.write('[INFO] Unzip the RWTH raw.zip file. There are'
               ' many levels of compression in this dataset.')
    subdir = path / 'RWTH-2021-04545_818642'
    if not (subdir / 'Rawdata.zip').exists():
        with zipfile.ZipFile(path / 'raw.zip', 'r') as zip_ref:
    ...
    with zipfile.ZipFile(subdir / 'Rawdata.zip', 'r') as zip_ref:
        for file in tqdm_wrapper(zip_ref.namelist(), desc=desc):
    ...
    for file in tqdm_wrapper(list(datadir.glob('*.zip')), desc=desc):
